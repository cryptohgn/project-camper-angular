...
constructor() {
  ...
  searchVan() {
    ...
    this.filteredVans = this.allVans.filter(
      (van) =>
        van.title
          .toLocaleLowerCase()
          .includes(this.formSearch.toLocaleLowerCase()) ||
        van.description
          .toLocaleLowerCase()
          .includes(this.formSearch.toLocaleLowerCase())
    );
  ...
  applyFilter = () => {
    let filterVans = this.allVans.filter((item) => {
      ...
      const brandFilter =
        ...
        this.filterForm.get('brand')!.value === item.brand;
      const modelFilter =
        !this.filterForm.get('model')!.value ||
        this.filterForm.get('model')!.value === item.model;
      const priceFilter =
        item.price >= this.filterForm.get('priceMin')!.value &&
        item.price <= this.filterForm.get('priceMax')!.value;
      const kmFilter =
        item.km >= this.filterForm.get('kmMin')!.value &&
        item.km <= this.filterForm.get('kmMax')!.value;
      const yearFilter =
        item.year >= this.filterForm.get('yearsMin')!.value &&
        item.year <= this.filterForm.get('yearsMax')!.value;
      const fuelFilter =
        (!this.filterForm.get('diesel')!.value &&
          !this.filterForm.get('gasoline')!.value &&
          !this.filterForm.get('gas')!.value &&
          !this.filterForm.get('electric')!.value) ||
        this.filterForm.get(item.fuel.toLowerCase())!.value;
      const doorsFilter =
        !this.filterForm.get('doors')!.value ||
        this.filterForm.get('doors')!.value === item.doors;
      const kwFilter =
        item.kw >= this.filterForm.get('kwMin')!.value &&
        item.kw <= this.filterForm.get('kwMax')!.value;

      const solarPanelFilter =
        !this.filterForm.get('solarpanel')!.value || item.solarpanel;
      const liftedRoofFilter =
        !this.filterForm.get('liftedRoof')!.value || item.liftedRoof;
      const backCamFilter =
        !this.filterForm.get('backCam')!.value || item.backCam;
      const androidIOAutoFilter =
        !this.filterForm.get('androidIOAuto')!.value || item.androidIOAuto;
      const specialFeaturesFilter =
        solarPanelFilter &&
        liftedRoofFilter &&
        backCamFilter &&
        androidIOAutoFilter;
      ...
      const changeFilter =
        (!this.filterForm.get('manual')!.value &&
          !this.filterForm.get('automatic')!.value) ||
        (this.filterForm.get('manual')!.value &&
          this.filterForm.get('automatic')!.value) ||
        (this.filterForm.get('manual')!.value && item.change === 'Manual') ||
        (this.filterForm.get('automatic')!.value &&
          item.change === 'Automatic');

      const thinsulateFilter =
        !this.filterForm.get('thinsulate')!.value ||
        item.isolation.isolationMaterial.thinsulate;
      const reflectixFilter =
        !this.filterForm.get('reflectix')!.value ||
        item.isolation.isolationMaterial.reflectix;
      const foamBoardFilter =
        !this.filterForm.get('foamBoard')!.value ||
        item.isolation.isolationMaterial.foamBoard;
      const kayflexFilter =
        !this.filterForm.get('kayflex')!.value ||
        item.isolation.isolationMaterial.kayflex;
      const other_isolationFilter =
        !this.filterForm.get('other_isolation')!.value ||
        item.isolation.isolationMaterial.other;
      const isolationFilter =
        !this.filterForm.get('isolation')!.value ||
        (thinsulateFilter &&
          reflectixFilter &&
          foamBoardFilter &&
          kayflexFilter &&
          other_isolationFilter);
